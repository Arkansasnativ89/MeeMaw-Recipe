    ...
    try {
        ...
        if (Array.isArray(data)) {
            ...
            data.categories.forEach(category => {
                category.recipes.forEach(recipeItem => {
                    // Check if it's the nested format (with improved_recipe/original_recipe)
                    // or flat format (recipe data directly in the item)
                    if (recipeItem.improved_recipe || recipeItem.original_recipe) {
                        // Nested format with original/improved recipes
                        const recipe = {
                            ...(recipeItem.improved_recipe || recipeItem.original_recipe),
                            id: recipes.length + 1,
                            _original: recipeItem.original_recipe,
                            _improved: recipeItem.improved_recipe,
                            _hasOriginal: !!recipeItem.original_recipe,
                            _hasImproved: !!recipeItem.improved_recipe
                        };
                        recipes.push(recipe);
                    } else {
                        // Flat format - recipe data is directly in the item
                        const recipe = {
                            ...recipeItem,
                            id: recipes.length + 1,
                            _original: recipeItem,
                            _improved: null,
                            _hasOriginal: true,
                            _hasImproved: false
                        };
...
function findBestImageMatch(recipeName) {
    ...
    const availableImages = [
        ...
        "CRANBERRY OR BLUEBERRY CRUNCH", "CRANBERRY ORANGE RELISH", "CRUMBLE CAKE (KRÜMELKUCHEN)", "CRUMBLE COFFEE CAKE #2",
...
function showRecipeDetail(recipe) {
    console.log('showRecipeDetail called with:', recipe);
    const modal = document.getElementById('recipeModal');
    const detailDiv = document.getElementById('recipeDetail');
    
    console.log('Modal element:', modal);
    console.log('Detail div:', detailDiv);
    ...
    console.log('Recipe _original:', recipe._original);
    console.log('Has both versions:', hasBothVersions);
    ...
    const formattedContent = formatRecipeContent(recipe._original || recipe, 'Original');
...
// View original cookbook image from the modal control bar
function viewOriginalImage() {
    const printDiv = document.querySelector('.cookbook-image-print');
    if (printDiv) {
        const imagePath = printDiv.getAttribute('data-image-path');
        if (imagePath) {
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function getCardWidth() {
        const card = carousel.querySelector('.memory-card');
        if (!card) return 0;
    ...
    function nextSlide() {
        const totalCards = carousel.querySelectorAll('.memory-card').length;
